...
voChartXY::voChartXY()
{
  this->ChartPrivate = new voChartXYPrivate;
...
//-----------------------------------------------------------------------------
bool voChartXY::Paint(vtkContext2D *painter)
{
...
//-----------------------------------------------------------------------------
void voChartXY::CalculateBarPlots()
{
...
//-----------------------------------------------------------------------------
void voChartXY::RecalculatePlotTransforms()
{
...
//-----------------------------------------------------------------------------
int voChartXY::GetPlotCorner(vtkPlot *plot)
{
...
//-----------------------------------------------------------------------------
void voChartXY::SetPlotCorner(vtkPlot *plot, int corner)
{
...
//-----------------------------------------------------------------------------
void voChartXY::RecalculatePlotBounds()
{
...
//-----------------------------------------------------------------------------
bool voChartXY::UpdateLayout(vtkContext2D* painter)
{
...
//-----------------------------------------------------------------------------
int voChartXY::GetLegendBorder(vtkContext2D* painter, int axisPosition)
{
...
//-----------------------------------------------------------------------------
void voChartXY::SetLegendPosition(const vtkRectf& rect)
{
...
//-----------------------------------------------------------------------------
vtkPlot * voChartXY::AddPlot(int type)
{
...
//-----------------------------------------------------------------------------
vtkIdType voChartXY::AddPlot(vtkPlot * plot)
{
  if (plot == NULL)
    {
    return -1;
...
//-----------------------------------------------------------------------------
bool voChartXY::RemovePlot(vtkIdType index)
{
...
//-----------------------------------------------------------------------------
vtkPlot* voChartXY::GetPlot(vtkIdType index)
{
...
//-----------------------------------------------------------------------------
void voChartXY::SetShowLegend(bool visible)
{
...
//-----------------------------------------------------------------------------
vtkChartLegend* voChartXY::GetLegend()
{
  return this->Legend;
}

//-----------------------------------------------------------------------------
vtkIdType voChartXY::GetNumberOfPlots()
{
  return this->ChartPrivate->plots.size();
}

//-----------------------------------------------------------------------------
vtkAxis* voChartXY::GetAxis(int axisIndex)
{
  if (axisIndex < 4)
    {
...
//-----------------------------------------------------------------------------
vtkIdType voChartXY::GetNumberOfAxes()
{
  return 4;
}
...
void voChartXY::AddPointLabels( std::vector< std::string> labels )
{
  ...
  it = this->ChartPrivate->plots.begin();
...
//-----------------------------------------------------------------------------
void voChartXY::RecalculateBounds()
{
  // Ensure that the bounds are recalculated
  this->PlotTransformValid = false;
  // Mark the scene as dirty
...
//-----------------------------------------------------------------------------
bool voChartXY::Hit(const vtkContextMouseEvent &mouse)
{
...
//-----------------------------------------------------------------------------
bool voChartXY::MouseEnterEvent(const vtkContextMouseEvent &)
{
...
//-----------------------------------------------------------------------------
bool voChartXY::MouseMoveEvent(const vtkContextMouseEvent &mouse)
{
...
//-----------------------------------------------------------------------------
bool voChartXY::LocatePointInPlots(const vtkContextMouseEvent &mouse)
{
  size_t n = this->ChartPrivate->plots.size();
  if (mouse.ScreenPos[0] > this->Point1[0] &&
      mouse.ScreenPos[0] < this->Point2[0] &&
        ...
        for (int j = items-1; j >= 0; --j)
          ...
          if (plot && plot->GetVisible())
            ...
            if (seriesIndex >= 0)
              ...
              //std::cout << "Found: " << plotPos[0] << "," << plotPos[1] << std::endl;
...
//-----------------------------------------------------------------------------
bool voChartXY::MouseLeaveEvent(const vtkContextMouseEvent &)
{
  this->DrawNearestPoint = false;
  this->Tooltip->SetVisible(false);
  return true;
...
//-----------------------------------------------------------------------------
bool voChartXY::MouseButtonPressEvent(const vtkContextMouseEvent &mouse)
{
  this->Tooltip->SetVisible(false);
  // Iterate through each corner, and check for a nearby point
  for (size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
...
//-----------------------------------------------------------------------------
bool voChartXY::MouseButtonReleaseEvent(const vtkContextMouseEvent &mouse)
{
  // Iterate through each corner, and check for a nearby point
  for (size_t i = 0; i < this->ChartPrivate->PlotCorners.size(); ++i)
    {
...
void voChartXY::ZoomInAxes(vtkAxis *x, vtkAxis *y, float *origin, float *max)
{
...
//-----------------------------------------------------------------------------
bool voChartXY::MouseWheelEvent(const vtkContextMouseEvent &, int delta)
{
  this->Tooltip->SetVisible(false);
...
//-----------------------------------------------------------------------------
bool voChartXY::RemovePlotFromCorners(vtkPlot *plot)
{
...
//-----------------------------------------------------------------------------
void voChartXY::PrintSelf(ostream &os, vtkIndent indent)
{
